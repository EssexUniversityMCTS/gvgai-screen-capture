...
public class App 
{
	...
	public static float[] preProcess(BufferedImage image, int block)
	{
	...
	/*	BufferedImage nm = new BufferedImage(im.length,im[0].length,BufferedImage.TYPE_INT_RGB);
		...
		*/
		...
		int[][] shrinkedIm = new int[width/block][height/block];
		
		System.out.println(im.length+" "+im[0].length);
		//System.out.println(res+" "+block*block);
		...
		*/
		...
		File outputfile = new File("max_"+block+".png");
	    try {
